...
def main(arglist):
	...
	### Set up group info
	## Regular design
    group_info = pd.read_csv('/Volumes/group/awagner/sgagnon/AP/scripts/subjects_groups.csv')
	# Subject source (no iterables here)
    subject_list = lyman.determine_subjects(args.subjects)

	## Subset of design
#     group_info = pd.read_csv('/Volumes/group/awagner/sgagnon/AP/scripts/subjects_groups_byshockCondSH.csv')
#     subject_list = group_info.subid.tolist() # make this for subset of subjects.txt
#     
...
#     subj_source.inputs.subject_id = subject_list
    ...
    group_vector = [1 if sub == "control" else 2 for sub in groups] # 1 for control, 2 for stress
    
    # Set up the regressors and contrasts
    regressors = dict(control_mean=[int(sub == 'control') for sub in groups], stress_mean=[int(sub == 'stress') for sub in groups])
    print regressors

    contrasts = [["control-stress", "T", ["control_mean", "stress_mean"], [1, -1]]]
#     contrasts = [["stress-control", "T", ["control_mean", "stress_mean"], [-1, 1]]]
...
def parse_args(arglist):
    ...
    parser.add_argument("-output", default="control-stress",
