...
%% Old code before eugenios fix to the z in rotated hippocampus 
% [Ym, Xm, Zm] = ind2sub(size(M.vol),find(M.vol>0));
% 
...
% minX = meanX + t(1)*dirVect';
% maxX =  meanX + t(2)*dirVect';
% 
% [PMin,IndMin] = min(score(:,1));
% [PMax,IndMax] = max(score(:,1));
% PuntoMin =  round(meanX + score(IndMin,:,:)*coeff'); % Punto mas extremo para plano
% PuntoMax = round(meanX + score(IndMax,:,:)*coeff'); % Punto mas extremo para plano
% 
% if ~(ismember(PuntoMin,Xo,'rows') && ismember(PuntoMax,Xo,'rows') )
...
% z_end=maxX(3);
% 
% % Compute initial Bezier curve (it is the PCA line)
...
% 
% 
% V0=[x_ini; y_ini;  x_int; y_int; x_end; y_end ];
...
% 
% if optim
%     [V,costFinal,info] = fminunc( @(xxx) hip_computeCostAndGrad(Xm,Ym,Zm,xxx,...
%                              z_ini,z_int,z_end, nSamples), opts.x0, opts);
% else                     
%     [V,costFinal,info] = lbfgsb( @(xxx) hip_computeCostAndGrad(Xm,Ym,Zm,xxx,...
%         z_ini,z_int,z_end, nSamples), -Inf*ones(size(V0)),Inf*ones(size(V0)),opts);
% end
% 
% x_ini=V(1);
% y_ini=V(2);
...
% if DEBUG  > 0
%     hold on
%     plot3(Xlist,Ylist,Zlist,'b','linewidth',5)
%     plot3([x_ini x_int x_end],[y_ini y_int y_end],...
%           [z_ini z_int z_end],'g.','markersize',30)
% end
% END of old code before fix
